...
condvar_data::condvar_data(thread_id_t thread_count, bool allow_spurious_wakeups)
    : ws_(thread_count)
{
...
condvar_data::~condvar_data()
{
    //!!! detect destoy when there are blocked threads
}

void condvar_data::notify_one(debug_info_param info)
{
    context& c = ctx();
...
void condvar_data::notify_all(debug_info_param info)
{
    context& c = ctx();
    //??? do I need this scheduler call?
    c.sched();
...
sema_wakeup_reason condvar_data::wait(mutex_wrapper const& lock, bool is_timed, debug_info_param info)
{
...
bool condvar_data::wait(mutex_wrapper const& lock, std::function<bool ()> const& pred, bool is_timed, debug_info_param info)
{
    context& c = ctx();
    sign_.check(info);
...
void condvar_data::event_t::output(std::ostream& s) const
{
