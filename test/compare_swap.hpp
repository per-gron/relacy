...
struct cas_spurious_fail_test : rl::test_suite<cas_spurious_fail_test<T>, 1, rl::test_result_until_condition_hit>
{
    ...
    {
        int cmp = 0;
        if (x.compare_exchange_weak(cmp, 1, rl::mo_seq_cst, $, rl::mo_seq_cst, $))
        {
            cmp = 1;
            if (x.compare_exchange_weak(cmp, 2, rl::mo_seq_cst, $))
            {
                cmp = 0;
                if (y.compare_exchange_weak(cmp, 1, rl::mo_seq_cst, $))
                {
                }
                else
                {
