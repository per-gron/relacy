...
waitset::waitset(thread_id_t thread_count)
    : set_(thread_count)
    , size_(0)
{
}

unpark_reason waitset::park_current(context& c,
                                    bool is_timed,
                                    ...
                                    debug_info_param info)
{
    ...
    {
        RL_VERIFY(ws[wsi]->size_ < set_.size());
...
bool waitset::unpark_one(context& c, debug_info_param info)
{
    if (0 == size_)
        return false;
...
thread_id_t waitset::unpark_all(context& c, debug_info_param info)
{
    thread_id_t cnt = 0;
...
thread_id_t waitset::size() const
{
    return size_;
}

waitset::operator bool () const
{
    return 0 != size_;
}

bool waitset::try_remove(context& c,
                         thread_id_t const idx,
                         debug_info_param info)
{
    RL_VERIFY(idx < size_);
